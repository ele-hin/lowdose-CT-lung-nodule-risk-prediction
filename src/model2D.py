...
class ConvolutionalBlock(nn.Module):
  def __init__(self,
    ...
    self.conv = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, groups=1, bias=False, padding_mode='zeros', 
                                        device=None, dtype=None),
                              nn.BatchNorm2d(out_channels),    # not used in original paper, but used in other papers and implementations, beneficial?
                              nn.ReLU(inplace = True),
                              nn.Conv2d(out_channels, out_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, groups=1, bias=False, padding_mode='zeros', 
                                        device=None, dtype=None),
                              nn.BatchNorm2d(out_channels),    # not used in original paper, but used in other papers and implementations, beneficial?
                              nn.ReLU(inplace = True),
    )
...
class UNet2D(nn.Module):
  def __init__(self,
    ...
    self.pool = nn.MaxPool2d(kernel_size = 2, stride = 2)     # use layer in between in forward method
