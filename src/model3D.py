...
class ConvolutionalBlock(nn.Module):
  def __init__(self,
    ...
    self.conv = nn.Sequential(nn.Conv3d(in_channels, out_channels, kernel_size, stride = 1, padding = 0, dilation = 1, groups=1, bias=False, padding_mode='zeros', 
                                        device=None, dtype=None),
                              nn.BatchNorm3d(out_channels),    # not used in original paper, but used in other papers and implementations, beneficial?
                              nn.ReLU(inplace = True),
                              nn.Conv3d(out_channels, out_channels, kernel_size, stride = 1, padding = 0, dilation = 1, groups=1, bias=False, padding_mode='zeros', 
                                        device=None, dtype=None),
                              nn.BatchNorm3d(out_channels),    # not used in original paper, but used in other papers and implementations, beneficial?
                              nn.ReLU(inplace = True),
    )
...
class unet(nn.Module):
  def __init__(self,
               in_channels = 3, out_channels = 1, #think about this?
